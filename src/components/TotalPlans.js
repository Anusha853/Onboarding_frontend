...
function TotalPlans() {
  ...
  /* Define plan ranges based on plan_type_id
  const personalPlans = plans.filter(plan => plan.plan_type_id === 1);
  const enterprisePlans = plans.filter(plan => plan.plan_type_id === 3);
  const businessPlans = plans.filter(plan => plan.plan_type_id === 2);
  const governmentPlans = plans.filter(plan => plan.plan_type_id === 4);*/
  // Define plan ranges
