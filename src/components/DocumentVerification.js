...
const DocumentVerification = () => {
    ...
    const fetchUserDetails = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          `${apiUrl}/user/profile`, // Use the environment variable here
          {
            username,
            password,
          }
  ...
  const handleUpload = async () => {
    ...
    try {
      const apiUrl = process.env.REACT_APP_BASE_URL;
      const res = await fetch(`${apiUrl}/user/document/upload`, {
        method: 'POST',
        body: formData,
      ...
      const verificationRes = await fetch(`${apiUrl}/user/document/status?userId=${userId}`);
